...
{
	dev_info(&sensor->i2c_client->dev, "ov5640_set_power_on start \n");
	...
	dev_info(&sensor->i2c_client->dev, "ov5640_set_power_on stop \n");
...
{
	dev_info(&sensor->i2c_client->dev, "ov5640_set_power_off start \n");

	ov5640_power(sensor, false);
	regulator_bulk_disable(OV5640_NUM_SUPPLIES, sensor->supplies);
	clk_disable_unprepare(sensor->xclk);
	
	dev_info(&sensor->i2c_client->dev, "ov5640_set_power_off stop \n");
}
...
static int ov5640_probe(struct i2c_client *client)
{
	...
	dev_info(dev, "Probe: Checkpoint 1\n");
